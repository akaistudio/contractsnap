...
def send_otp_email(email, code, purpose='login'):
    ...
    html = f"""<div style="font-family:sans-serif;max-width:400px;margin:0 auto;padding:24px">
        <h2 style="color:#2563eb">ContractSnap</h2>
        ...
        <p style="color:#999;font-size:11px;margin-top:20px">Part of <a href="https://snapsuite.up.railway.app" style="color:#2563eb">Varnam Suite</a></p>
    </div>"""
    if not resend_key:
...
def register_with_hub(company_name, email, currency):
    ...
    try:
        http_requests.post(f'{hub}/api/register-company', json={
            'app_name': 'ContractSnap', 'company_name': company_name,
            'email': email, 'currency': currency,
            'app_url': 'https://contractsnap-app.up.railway.app'
        }, timeout=5)
    except: pass
